...
| `amsc.methods` | **The chunking-method registry** — one `ChunkMethod` per method (wire key, engine kind, product label, summary, capabilities, and for partition methods the `partition` callable). The Viewer v3/v2 builders, the chunk benchmark, the relation deriver and the chat_rag console read their method lists from it; `amsc.example_chunker` is the unregistered template. Adding a method = write a partition + one `ChunkMethod` in `_BUILTIN` + a test (`docs/adding-a-chunker.md`). Deep Analysis is registered as an orchestration (`deep=True`, `baseline="structure-only"`), never as a partition |
