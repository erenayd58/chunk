...
| `amsc.viewer_corpus` | **The Viewer's document reader** — artifact trees become the per-document payload a Viewer page renders (`load_corpus`), plus the arm index the chat server serves (`catalog`). The Viewer page and `chat_rag`'s packaging worker both read it, so a live document and a frozen one have the same shape. Pure reader: renders no page, verifies every canonical pin, states no method name of its own (`amsc.methods` owns identity). `load_corpus(units_path=…, extra_arm_dirs={arm: dir})` is how the live workspace assembles a document from one canonical and one packaged arm per method. [docs/viewer-architecture.md](docs/viewer-architecture.md) |
...
| `amsc.viewer_v3` | **The Viewer — the chunking inspection product page**, the one `start-demo.ps1` builds and serves. A page builder over the shared reader: per-document payloads are verbatim `viewer_corpus.load_corpus` output, embedded at build time or fetched at runtime for a live document. Methods are never hard-coded — exactly the document's packaged arms are offered, under the registry's product names. Build with no arguments at all for the **product shell**: no embedded corpus, every document read live from the console, and the only build a fresh clone can make. `--benchmark DOC=DIR` / `--deep DOC=DIR` / `--label DOC=LABEL` embed frozen trees instead. Template in `viewer_v3_template.py`; [docs/viewer-v3.md](docs/viewer-v3.md) and [docs/viewer-architecture.md](docs/viewer-architecture.md) |
