...
| `amsc.viewer_v2` | Presentation-grade single-file HTML viewer over a completed chunk-benchmark tree (Sunum/Sorgu/Debug/Benchmark modes); pure reader, verifies the tree's `units_sha256` pin. Optional `--agentic DOC=DIR` adds the Agentic Chunker as a fourth arm from an `agentic_chunker` tree — it rides in `arms` but never enters `armOrder`, so the frozen dashboard tables and the three-arm difference definition are untouched and a build without the tree is byte-identical; a page-sliced smoke tree or a different canonical is refused |
