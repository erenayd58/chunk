...
| `amsc.chunking.registry` | **The chunking-method registry** — one `ChunkMethod` per method (wire key, engine kind, product label, summary, capabilities, and for partition methods the `partition` callable). The Viewer v3/v2 builders, the chunk benchmark, the relation deriver and the chat_rag console read their method lists from it; `amsc.chunking.example` is the unregistered template and `amsc.chunking.method` is the leaf module its types come from (so the registry can import a method module without a cycle). Adding a method = one method module (partition + its `ChunkMethod`) + one import and one tuple element in `_BUILTIN` + a test (`docs/adding-a-chunker.md`); no test pins the method count, and a served Viewer reads the registry at runtime, so no page rebuild. Deep Analysis is registered as an orchestration (`deep=True`, `baseline="structure-only"`), never as a partition |
