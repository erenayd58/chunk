...
| `amsc.viewer_corpus` | **The Viewer's document reader** — the one place artifact trees become the per-document payload a Viewer page renders (`load_corpus`) plus the arm index the chat server serves (`catalog`). Extracted out of `viewer_v2` in Phase 6: both pages and `chat_rag`'s packaging worker read it, so a live document and a frozen one have the same shape and nothing that touches a payload has to import a 230 KB page template. Pure reader, renders no page, verifies every canonical pin; method identity comes from `amsc.methods`. `load_corpus` also takes `units_path=` + `extra_arm_dirs={arm: dir}`, which is how the live workspace assembles a document from **one canonical and one packaged arm per chunking method**. See [docs/viewer-architecture.md](docs/viewer-architecture.md) |
| `amsc.viewer_v2` | **Compatibility: the earlier product page**, kept for the research build (its `--agentic` provenance arm) and as a manual fallback; Viewer v3 is what `start-demo.ps1` builds and serves. A *page builder only* since Phase 6 — every payload it renders comes from `amsc.viewer_corpus`, and the reader names callers still import from here (`load_corpus`, `ARM_KINDS`, `display_html`…) are re-exports of that module's own objects. A single offline HTML with four modes: Sunum (opens on **the comparison board**: one column per selected method, ordered the way the reader picked them, first pick leftmost. The paragraph is the atom -- the same paragraph is printed into the same grid row in every column -- so only the boundaries move. A chunk is a card: it opens on a boundary row carrying its number and its reason (`Parça 74 · Boyut sınırına ulaşıldı · 371 tk`), runs through the paragraph rows it owns, and closes on the next boundary row; a method that did NOT cut runs a hatched card straight through the same height saying `Parça 73 sürüyor`. That row is a **divergence**: a numbered button in the index column jumps to it, and a readout above the board says it in words plus Deep's decision at that boundary when the pipeline recorded one. Opens on the first divergence, steps with the buttons or ← →, brings the chunks that meet there into view and draws them in their method's colour. The control strip carries three things and no more -- the methods, `← Ayrışma x / 43 →`, and the page; text density, the fold, the Standard/Deep filter and the link down to the result band live behind one **Görünüm** button, and there is no second index beside the columns' own divergence numbers. A card says its number and a two-word reason; its size, section and decision are in a **drawer** that opens on a picked chunk and gives the width straight back. The readout above the board is one sentence ("Deep Analysis burada yeni parça açtı · Standard devam etti") with the pipeline's reasoning behind **Neden?**. Ownership is read from `arm.seg`, never from `arm.m` alone -- `m` records only the first chunk containing a unit and drops the headings a method keeps out of its unit ids, so a card drawn from `m` puts Markdown's boundaries in the wrong places. **The row decides, not the cell**: a paragraph one method cuts inside is sliced at the UNION of every column's offsets, a note line is drawn in every column whenever any column has one, and no mark may change how wide the text is -- measured, one 11px padding in one column moved 1141 rows out of true. The Standard→Deep result band, the "what improved" chart, the method cards and the methodology disclosure follow below the board). One shell for the four screens: a sticky header (brand, underline tabs, document picker, console button) and a **page head** on every screen -- which screen, which document, what for -- over a cool neutral palette with one blue for the product and one violet for Deep Analysis; a method wears the same colour on its card, its column chip and its chunk heads, and clicking a card adds or removes its column. The divergence chrome is ink, never a method colour., Sorgu (live "ask the document" chat when served by `viewer_server`, frozen gold-query view offline), Debug (units, mappings, parser findings, representation ceilings, the Deep decision trail per section -- the one tab that keeps the engine's own vocabulary), Benchmark (frozen v5 tables untouched + a separate Deep Analysis panel + cross-document contract table). Inputs per document: `--benchmark DOC=DIR` (frozen v5 tree, optional), `--deep DOC=DIR` (a `deep_run` tree packaged by `deep_arm`; alone it builds a Standard-vs-Deep document), `--agentic DOC=DIR` (the earlier research tree, provenance only; not combinable with `--deep`). `load_corpus` also takes `units_path=` + `extra_arm_dirs={arm: dir}`, which is how the live workspace assembles a document from **one canonical and one packaged arm per chunking method** -- a document is a canonical plus the set of arms actually produced for it, and nothing else is rendered. Pure reader; verifies every canonical pin; writes `catalog.json` beside the page (documents, arms, chunk files **and the build inputs**, which is what pins the published page to a fresh build). **One name per method everywhere** (Standard, Deep Analysis); the engine name stays as a `techname` beside it wherever a number is audited, and Debug is the one tab that keeps the engine's own vocabulary. **One place decides what actually ran**: `analysisState()` maps the pipeline's five statuses plus the *attempted*-call count onto the six things a reader must tell apart — a model-backed run, a partly-answered one, a rules-only upload, a Deep run nothing was uncertain enough to ask about, an unreachable provider, and a provider that answered nothing — and every badge, verdict, cost figure and guarantee sentence reads that one function, so no screen can price a model that never ran. The template lives in `viewer_v2_template.py` |
...
| `amsc.methods` | **The chunking-method registry** — one `ChunkMethod` per method (wire key, engine kind, product label, summary, capabilities, and for partition methods the `partition` callable). The Viewer v3/v2 builders, the chunk benchmark, the relation deriver and the chat_rag console read their method lists from it; `amsc.example_chunker` is the unregistered template and `amsc.chunk_method` is the leaf module its types come from (so the registry can import a method module without a cycle). Adding a method = one method module (partition + its `ChunkMethod`) + one import and one tuple element in `_BUILTIN` + a test (`docs/adding-a-chunker.md`); no test pins the method count, and a served Viewer reads the registry at runtime, so no page rebuild. Deep Analysis is registered as an orchestration (`deep=True`, `baseline="structure-only"`), never as a partition |
| `amsc.viewer_v3` | **Viewer v3 — the chunking inspection product page**, the page `start-demo.ps1` builds and serves. A page builder over the shared reader: per-document payloads are verbatim `viewer_corpus.load_corpus` output (embedded at build time; live documents arrive with the same shape through the server's existing `/api/workspace` + `/api/live-document` relays), and the catalog writer is v2's. One question first: where does a chunk start and end, and how do two methods cut the same content differently. Opens empty (no KB, document or method preselected; a compact selection bar carries *Bilgi tabanı › Doküman › Yöntemler*); methods are never hard-coded — exactly the document's packaged arms (live: `live.methods` ready ones) are offered under product names (Markdown, Hybrid, Standard, Deep Analysis). Single-method view is a reading page: one paper sheet per PDF page, a boundary = whitespace + hairline + a small quiet label (`74 · Boyut sınırı`); clicking a chunk opens a minimal popover (pages, section, tokens, human-language boundary reason; technical detail behind a disclosure). A second method splits the same sheet into columns sharing one CSS grid — the same canonical unit prints into the same row everywhere, a unit cut inside is sliced at the union of every column's `seg` offsets, so only the boundaries move; a slim gutter marks rows where the selected methods diverge and `‹ Fark / Fark ›` walks them across pages. Deep Analysis chrome only where a packaged run exists: a violet dot on boundaries its decision story touched, the fix named in the popover, the chip carrying `analysisState`-style notes (`kural tabanlı`, `modelsiz tamamlandı`…) — no dashboard. Build with no arguments at all for the **product shell** — no embedded corpus, every document read live from the console, and the only build a fresh clone can make (`start-demo.ps1` runs it when the page is missing); or with `--benchmark`/`--deep`/`--label` (same specs as v2) to embed frozen trees, to `artifacts/viewer-v3/index.html`; serve with the unchanged `amsc.viewer_server --viewer artifacts/viewer-v3/index.html`. Template in `viewer_v3_template.py`; docs in `docs/viewer-v3.md` and [docs/viewer-architecture.md](docs/viewer-architecture.md) (ownership across both repos, build inputs, packaging lifecycle, debugging a failed package) |
...
- [Viewer mimarisi](docs/viewer-architecture.md) — Viewer ownership across `chunk` and `chat_rag`, build inputs/outputs, the packaging lifecycle, V2 status, how a new chunker reaches the Viewer, debugging a failed package
