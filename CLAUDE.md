...
```powershell
py -3.11 -m pytest
py -3.11 -m pytest tests/unit/chunking/test_v4_selection.py
py -3.11 -m pytest tests/unit/chunking/test_v4_selection.py::test_name
...
| Module | Purpose |
|---|---|
| `amsc.research.corpus.prepare_pages` | PDF → canonical JSONL for selected pages (`--pages 40-55`) |
| `amsc.canonical.prepare` | Full mixed-orientation document, requires `--layout-profile`; `--canonical-profile v1-frozen\|v2-repaired\|v3-semantic` |
| `amsc.research.corpus.qa` | Human-readable QA preview of canonical JSONL |
| `amsc.quality.evaluation` | `worksheet` / `evaluate` — the **authoritative** boundary + chunk metrics |
| `amsc.research.failure_analysis` | Prediction/gold/merge audit, `--run ID=DIR` (diagnostic only) |
| `amsc.research.phase3c` | Phase 3C semantic comparators C0–C3 |
| `amsc.research.v5` | Phase 3B scale-calibration B0–B3 |
| `amsc.research.benchmark.run_retrieval` | Phase 4 retrieval benchmark on KKB 2024 |
| `amsc.research.benchmark.run_holdout` | Phase 5 holdout validation on KKB 2022 |
| `amsc.research.benchmark.chunkers` | Markdown / Hybrid / Structure-only comparison on time and quality (`--output` is **required**) |
| `amsc.canonical.refine.semantic_roles` | Library only — the heading/section split; no CLI |
| `amsc.quality.lint` | Structural QA linter over a canonical corpus and its chunks |
| `amsc.viewer.corpus` | **The Viewer's document reader** — artifact trees become the per-document payload a Viewer page renders (`load_corpus`), plus the arm index the chat server serves (`catalog`). The Viewer page and `chat_rag`'s packaging worker both read it, so a live document and a frozen one have the same shape. Pure reader: renders no page, verifies every canonical pin, states no method name of its own (`amsc.chunking.registry` owns identity). `load_corpus(units_path=…, extra_arm_dirs={arm: dir})` is how the live workspace assembles a document from one canonical and one packaged arm per method. [docs/viewer-architecture.md](docs/viewer-architecture.md) |
| `amsc.viewer.server` | Stdlib HTTP server that serves the viewer and the chat API (`/api/health`, `/api/chat`, `/api/retrieve`, `/api/compare`, `/api/chunk`) from the viewer's `catalog.json` + `configs/rag-poc.yaml`. Keys never reach the page: the browser talks to this process only. `--warm` builds every index first; `--lexical` runs BM25-only; `--no-answer` retrieval-only. `--console-url` (env `AMSC_CONSOLE_URL`, default `http://127.0.0.1:5005`) additionally serves `/api/workspace`, `/api/live-document?doc=` and `POST /api/live-prepare` — server-side reads of `chat_rag`'s `/api/demo/workspace` and `/api/demo/viewer-analysis/<doc>`, so every console document whose analysis is ready appears in the page's own document picker -- one navigation for the frozen corpus and the live workspace alike -- and opens without a second origin, a CORS grant or a copy of that state. The console listing itself is a top-bar button and a dialog, never content. The console packages those documents itself (its own ingest's canonical + `deep.run.write_tree` + `deep.arm.package` + `load_corpus`), so no PDF is parsed twice and no second proposer/verifier call is ever made; the page merges the payload into `DATA.docs` flagged **live**, which keeps it out of the frozen benchmark tables, the three-arm comparison and the cross-document table, and out of any Hit@k/MRR claim it has no gold set for. An unreachable console is a rendered state, not an error |
| `amsc.surface` | **The library surface declaration** — which modules are `product` (derived: whatever a declared entry point reaches by a module-level import), `service` (the Viewer's own server process), `research`, `legacy` or `unused`, plus `CONSOLE_API`: the 21 modules `chat_rag`'s product code may import. The one rule — *nothing reachable from an entry point may be research, legacy or unused* — is checked against the real import graph by [test_library_surface.py](tests/unit/test_library_surface.py), and the console's side by `chat_rag/tests/unit/test_amsc_surface.py`. `MIXED` names the four product modules with a research heritage and exactly what product uses from each. Read [docs/library-surface.md](docs/library-surface.md) before adding a module or a new `from amsc...` import in the console |
| `amsc.chunking.registry` | **The chunking-method registry** — one `ChunkMethod` per method (wire key, engine kind, product label, summary, capabilities, and for partition methods the `partition` callable). The Viewer v3/v2 builders, the chunk benchmark, the relation deriver and the chat_rag console read their method lists from it; `amsc.chunking.example` is the unregistered template and `amsc.chunking.method` is the leaf module its types come from (so the registry can import a method module without a cycle). Adding a method = **one file** in `amsc/chunking/plugins/` (a partition returning `contract.Chunk` objects under `@chunker(...)`) + a test; `amsc.chunking.discovery` imports the directory, so no list is edited (`docs/adding-a-chunker.md`). `amsc.chunking.contract` is the one contract layer: four core row fields, everything else a declared `Capability` the framework derives; no test pins the method count, and a served Viewer reads the registry at runtime, so no page rebuild. Deep Analysis is registered as an orchestration (`deep=True`, `baseline="structure-only"`), never as a partition |
| `amsc.viewer.build` | **The Viewer — the chunking inspection product page**, the one `start-demo.ps1` builds and serves. A page builder over the shared reader: per-document payloads are verbatim `viewer.corpus.load_corpus` output, embedded at build time or fetched at runtime for a live document. Methods are never hard-coded — exactly the document's packaged arms are offered, under the registry's product names. Build with no arguments at all for the **product shell**: no embedded corpus, every document read live from the console, and the only build a fresh clone can make. `--benchmark DOC=DIR` / `--deep DOC=DIR` / `--label DOC=LABEL` embed frozen trees instead. Template in `viewer/template.py`; [docs/viewer-v3.md](docs/viewer-v3.md) and [docs/viewer-architecture.md](docs/viewer-architecture.md) |
| `amsc.viewer.chat.session` / `viewer.chat.index` / `viewer.chat.context` / `viewer.chat.answer` / `retrieval.embeddings` | The RAG engine behind Sorgu: one hybrid index per (document, arm) — dense (configurable OpenAI-compatible embeddings, reference `qwen/qwen3-embedding-8b`, or a local sentence-transformers model; per-text `.npy` cache under `.cache/rag-embeddings/<model>/`) + BM25 (the benchmark's Turkish fold) + RRF via the frozen `DeterministicHybridIndex`; context assembly walks `chunking.relations.expand_context` (same-section budget continuations only), dedups, spends a token budget in rank order and labels sources `S1..Sn`; the answer model is prompted to cite labels and to say when sources are insufficient, replying JSON. Same retriever for every arm, so a `compare` varies the chunker only. Provider failures degrade (BM25-only index, `answer_error` with sources still shown) rather than raise |
| `amsc.deep.pipeline` | **The production-facing Deep Analysis entry point** (in-memory, no files): `chunk_document(units, mode="standard"\|"deep", settings=DeepAnalysisSettings(...))` → `ProductChunkingResult(rows, status, report)`; `run_deep_analysis` is the full pipeline (Standard → proposer → selector → double-order verifier → measure) that `deep.run` wraps for the CLI. Failure policy: missing key → `fallback_no_provider` (deterministic contract alone), every call failing → `fallback_provider_error`, partial failure → `degraded`; a structural error still raises. `build_providers` reads only whether the key variable is set. This is the entry point `chat_rag` should call instead of `research.agentic.judge.chunk_with_product_mode` |
| `amsc.deep.arm` | Packages a `deep.run` tree for the viewer and the benchmark panel: `arm/` (chunks with canonical unit ids, mapping, structural quality, and — with `--frozen-tree` — retrieval/query-results/timing from the frozen evaluator under the frozen BM25 settings and gold set), `standard/` (the same for the Standard partition), and `boundary-decisions.json`: the deterministic **decision story** — per section `standard_kept` / `deterministic_improved` / `llm_accepted` / `llm_reverted` / `contract_reverted`, per final cut its origin (standard / deterministic / llm), the smells the moved cut removed, size effects, verifier verdicts, and representation-ceiling cuts (inside one unit). The deterministic cuts are recomputed (LLM-free selector) so the story is derivable from a tree that only recorded the final partition |
| `amsc.chunking.relations` | Derived continuation sidecars over frozen chunks (adjacent + same heading + joining section paths, nothing else), typed by the observable boundary: `TOKEN_BUDGET_CONTINUATION` only for a plain `budget_split`; label seams and markdown cuts get their own types. `expand_context` walks **only** TOKEN_BUDGET_CONTINUATION — never re-ranks, hard token budget, section change or any non-budget boundary is a hard stop |
| `amsc.research.semantic_assist` | Library only — **embedding-assisted research baseline** (not the product mode): `STANDARD` delegates to `chunking.structural`, `SEMANTIC_ASSIST` to `chunking.hybrid` (both byte-identical to the benchmarked arms); `eligible_sections` names the ambiguity surface embedder-free. `OpenRouterEmbeddingProvider` is adapter-only, NOT VERIFIED; Qwen3-Embedding-8B is reserved as a future *retrieval* embedding candidate |
| `amsc.research.agentic.judge` | Library only — the product's **Deep Analysis** mode: structure-first chunking whose plain budget cuts with ≥2 admissible positions are judged SPLIT/KEEP by a backend generative LLM at ingest. **One provider call per decision window** (v2 batching): every admissible candidate is marked `[CANDIDATE Cn]` in a single shared-context prompt and the model answers a JSON array with one decision per candidate — it is never offered the final selection; latest-SPLIT-wins / all-KEEP-greedy stay client-side rules, and a missing, duplicate or unknown `candidate_id` refuses the whole window. Label seams stay structural; no judge / all-KEEP / parse or provider failure is byte-identical to `chunking.structural` (pinned). Structured decisions only (`reason_code` is audit, never steering); provider-agnostic `providers.OpenAICompatibleJudgeProvider` (model id required, key env configurable, NOT VERIFIED); backend-only — no key or LLM call ever reaches viewer HTML |

| `amsc.research.agentic.chunker` | The **Agentic Chunker** — the fourth research arm: section-annotate → parallel vote collection → vote-guided deterministic walk. The structural walk is unchanged; per oversized section whose all-KEEP dry-walk would consult ≥1 multi-candidate window, ONE prompt marks every internal non-label boundary `[CANDIDATE Cn]` (segments of ≤24 past the caps), all calls run concurrently, and a plain budget cut with ≥2 admissible positions takes the latest effective-SPLIT-voted position, else greedy. **In this arm `reason_code` steers, but only as a veto**: SPLIT+continuation-reason demotes to abstain; demoted SPLITs `> max(2, ceil(0.20·n))` reject the call (`coherence_violation`). No votes / all-KEEP / any failure ≡ `chunking.structural` byte for byte; hard cap and label seams untouched. **Rendering invariant:** a chunk's text is a function of its ordered `unit_ids` — a unit list equal to a Structure-only chunk's carries that chunk's exact text/token_count, any other shape renders heading-once — so a provisional LLM cut absorbed by the rejoin leaves no duplicated heading; it stays on record as `rejoined_after_agentic_cut` and is never counted in `final_boundary_moved_count` (window-level `window_moved_count` is reported separately; `boundary-diff.json` counts final boundaries). Artifacts carry `prompt_sha256` + responses, never raw prompt text (replay = deterministic prompt reconstruction; `--dump-prompts` is local-only). Prompt-set stability holds for same canonical **and** same config/candidate plan only. All knobs `poc_initial_not_optimized`; results model-dependent, replay-deterministic only |
| `amsc.research.benchmark.agentic` | Separate evaluation runner for the agentic arm: reads BM25/top_ks/token/budget settings from the frozen tree's own `resolved-config.json`, scores the agentic chunks with the frozen metric imports, writes into the agentic tree, and copies the frozen v5 numbers **verbatim** (with the summary file's sha) into `comparison-summary.json` — never recomputes them. Refuses page-sliced smoke trees, mismatched canonicals, stale mappings, and any output inside `evaluation/` or a frozen benchmark tree. Agentic chunking time is provider-bound and deliberately not comparable with frozen `chunk_ms` |
| `amsc.quality.boundaries` | Library + CLI — **deterministic boundary smells** for the structure-first family (Kademe 0 of the Deep Analysis v2 plan). Shape-only predicates, no lexicon: `orphan_label` (non-opening, non-`display` heading or emphasis-wrapped short paragraph at a chunk tail), `lead_in_cut` (tail ends with `:`), `fragment_cut` / `table_split` (one unit across two chunks), `run_split_when_fits` (a list run that fits `target` split), `continuation_cut` (head starts lower-case or with a footnote marker), plus `below_min` / `above_soft_max` counters. Smells attach to **sections** on final blocks; `compare` applies the contract `count_D[t] <= count_S[t]` per type (`worse` / `tie` / `better`) and emits **change groups** (spans between cuts common to both partitions) — the unit a verifier accepts or reverts. Anaphora/topic are deliberately absent (LLM domain). Refuses to write into `evaluation/` |
| `amsc.providers` | Library — **how a generative provider is called**, and nothing about chunking: the `BoundaryJudgeModel` protocol, `OpenAICompatibleJudgeProvider` (model id required, key env configurable, key read at request time and never persisted, NOT VERIFIED against a live service), `collect_votes` (one `complete()` per planned call on a pool, cache-first by `prompt_sha256`, `provider=None` is deterministic replay, one failure never takes the batch down, results in plan order) and `load_response_cache`. Deep Analysis's proposer and verifier run on this; the Agentic arm and the v1 judge import it too, which is what keeps both of them off the product path |
| `amsc.deep.selector` | Library + CLI — the **Deep Analysis** selector, the product's premium mode. Structure-first pieces, then a per-section lexicographic DP whose first cost term is the `quality.boundaries` smell count, so optimiser and metric cannot drift: `(smells, forbidden, -strength, below_min, above_soft_max, cuts_differing_from_standard, size_deviation)`. Standard's own partition is an explicit candidate and **wins every tie**; `standard_groups` restates the frozen walk's split+rejoin per section and is pinned byte-identical against `chunking.structural` (the one cross-section rejoin is restored by `_rejoin_across_sections`, never across a moved section). The admissible band is no longer `[min, target]` — any piece boundary whose blocks fit the hard cap is a candidate, which is what reaches the forced single-candidate cuts a vote-based design could never touch. **V0 gate** per section: no smell type may grow (tier 1); a size counter may grow only as the price of a strictly smaller smell total (tier 2, `compare_tiered`), else a conservative re-solve that forbids every new smelly cut, else revert to Standard. A model reaches the objective only through `BoundaryVote` (`strength` 0-3 + left/right roles) at cost slots 2-3, **below every smell term**, so no vote can buy a defective boundary; with no votes the run is a pure function of the canonical. All knobs `poc_initial_not_optimized` |
| `amsc.deep.proposer` | Library — the Deep Analysis **proposer**: one prompt per section that still has a choice to make, asking per candidate boundary for `strength` 0-3 plus two role fields (`before: finished\|introduces_next`, `after: standalone\|continues_previous`) instead of a binary SPLIT/KEEP. Size is never mentioned, the model never sees the partition, and **only boundaries the deterministic layer would accept are marked** (a cut that strands a lead-in is not offered). Strict parse: a missing, duplicate or unknown id refuses the whole call; a role value written on the wrong side is read as neutral, because it describes that piece's *other* boundary. Reuses `providers.collect_votes` for cache-aware parallel calls; artifacts carry `prompt_sha256` + responses, never prompt text |
| `amsc.deep.run` | CLI — plan, collect, select, measure, write, in one tree for all three modes (`--no-llm` / `--replay DIR` / `--model ID`, reference `qwen/qwen3-30b-a3b-instruct-2507`). Refuses `evaluation/` and any frozen benchmark tree; the key is read from the env at request time and never stored. Note prompts depend on the *config* as well as the canonical, so changing a smell rule changes the marked boundary set and is a deliberate cache miss |
| `amsc.deep.verifier` | Library — the Deep Analysis **verifier**. Nothing the proposer moves is kept on its own say-so: each **change group** (the span between two cuts both partitions agree on) is shown as two finished alternatives in full, twice, with the alternatives swapped, and the proposal is kept only when it wins **both** orders. Order-dependent answers are position bias, not judgement, and revert — on KKB 2024 the two orders disagreed on 23 of 46 groups. The group, never a single cut, is the unit of decision; reverting to the deterministic partition is always available; and the `deep.selector` contract runs again afterwards, so even a unanimous mistake cannot breach the hard cap or add a smell. Blind labelling of the accepted changes: 10 preferred / 3 not / 1 unacceptable over 13 groups, against the raw proposer's 22 / 14 / 7 |
| `amsc.research.gold.boundary_preference` | Library + CLI — blind human **boundary-preference** labelling: `build` makes `items.json` (manifest, carries the A/B blinding) + a self-contained `form.html` (never carries blinding or deterministic verdicts) from a Standard and a Deep partition: every change group (full text of both partitions, A/B by sha256 of the item id), a deterministic sample of unchanged multi-candidate windows, and every smelly forced cut; `score` unblinds exported labels and reports `preferred_or_equal_rate`, `worse_than_standard_rate`, acceptability and reason histograms. `enumerate_windows` is a read-only mirror of the structural walk (multi / forced / label_seam), pinned against `chunking.structural` by test. No RNG, no external resources |

Both preparers are thin CLI wrappers over an in-memory core —
`canonical.adapter.extract_canonical_units()` and
`canonical.prepare.extract_full_canonical_units()` — which return a
...
`canonical.prepare.V2_CANONICAL_REPAIRS`: visual-grid reconstruction, lead-in and
...
[canonical/refine/heading_levels.py](src/amsc/canonical/refine/heading_levels.py). `v3-semantic` adds
[canonical/refine/semantic_roles.py](src/amsc/canonical/refine/semantic_roles.py) on top. Each writes `data/*.units.v{2,3}.jsonl`
and records its repair set under `canonical_profile` in the manifest. [canonical/refine/split_headings.py](src/amsc/canonical/refine/split_headings.py) carries two rejoins that are geometric
...
[canonical/refine/semantic_roles.py](src/amsc/canonical/refine/semantic_roles.py) separates them: every heading gets a
...
`quality.lint.check_section_consistency` reads that same invariant through one predicate,
...
`canonical.refine.heading_levels.assign_heading_levels` exists because PyMuPDF4LLM's layout backend writes
...
[research/gold/repin.py](src/amsc/research/gold/repin.py) carries a gold set across **only** when every
...
**[docs/package-layout.md](docs/package-layout.md) is the map**: which package owns what,
which way imports point, and where a new module goes. Read it before adding a file.
...
V4 threshold-relative selection + soft structural support + semantic-safe merge. The whole
lineage lives in [src/amsc/chunking/adaptive/](src/amsc/chunking/adaptive/). V1–V3 share
`v1_v3.py`; **V4 has its own orchestration** ([v4.py](src/amsc/chunking/adaptive/v4.py),
[chunking/adaptive/v4_selection.py](src/amsc/chunking/adaptive/v4_selection.py), [structure.py](src/amsc/chunking/adaptive/structure.py),
[strength.py](src/amsc/chunking/adaptive/strength.py), [merge.py](src/amsc/chunking/adaptive/merge.py)) precisely so V4
conditionals never leak into the V1–V3 facades. Keep it that way — do not add version
branches to [v1_v3.py](src/amsc/chunking/adaptive/v1_v3.py).

Seams that exist to be swapped without touching orchestration:

- `TokenCounter` ([tokenization.py](src/amsc/document/tokenization.py)) is deliberately independent of
  the embedding tokenizer. When the production tokenizer is learned, only the adapter changes.
- `SemanticThresholdEstimator` ([thresholds.py](src/amsc/chunking/adaptive/thresholds.py)) and
  `SemanticFeatureExtractor` ([features.py](src/amsc/chunking/adaptive/features.py)) are protocols; the selector
...
[research/benchmark/chunkers.py](src/amsc/research/benchmark/chunkers.py) compares three arms — `markdown_recursive`
([chunking/markdown.py](src/amsc/chunking/markdown.py)), `hybrid_h1`
([chunking/hybrid.py](src/amsc/chunking/hybrid.py)) and `structure_first`
([chunking/structural.py](src/amsc/chunking/structural.py)) — over one frozen canonical input
with BM25-only retrieval, so only the chunker varies. It **imports** the frozen metric
functions from `research/benchmark/retrieval.py` rather than restating them, which imposes four
...
[chunking/mapping.py](src/amsc/chunking/mapping.py) is the single chunk↔unit resolver — an
offset/provenance/normalized/sequential ladder that reports `unmapped` rather than guessing —
and both [quality/chunks.py](src/amsc/quality/chunks.py) and
[research/benchmark/inspector.py](src/amsc/research/benchmark/inspector.py) consume it. `quality.chunks` subtracts a
`lint(units, [])` parser baseline before comparing arms, because most `quality.lint` findings
are properties of the parser and identical across arms.

Config ([config.py](src/amsc/chunking/adaptive/config.py)): all models are `extra="forbid"`; `load_config()`
...
Canonical input contract ([io.py](src/amsc/document/io.py), [models.py](src/amsc/document/models.py)): exactly
...
`amsc.research.benchmark.run_holdout` writes to by default, so a default run overwrites its own frozen
...
**`evaluate_checkpoint()` in [evaluation.py](src/amsc/quality/evaluation.py) is the single
authoritative metric.** Layers like [research/failure_analysis.py](src/amsc/research/failure_analysis.py)
...
- [Library surface](docs/library-surface.md) — product / research / legacy statuses, what `chat_rag` may import, where new code goes, and why the files did not move
- [Viewer mimarisi](docs/viewer-architecture.md) — Viewer ownership across `chunk` and `chat_rag`, build inputs/outputs, the packaging lifecycle, how a new chunker reaches the Viewer, debugging a failed package
