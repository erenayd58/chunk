...
| Sekme | Soru | İçerik |
|---|---|---|
| **Sunum** | Ne yaptık, fark ne? | İlk ekran **Parçaları karşılaştır** tezgâhıdır: seçilen her yöntem bir **kolon**dur ve kolonlar seçim sırasına göre dizilir (ilk seçilen solda; varsayılan iki yöntem). Paragraf ortak birimdir: aynı paragraf her kolonda aynı satırda basılır, böylece yalnız **parça sınırları** oynar. Bir parça karttır — bir sınır satırında numarasıyla ve iki kelimelik nedeniyle açılır (`Parça 74 · boyut sınırı`), sahip olduğu paragraf satırları boyunca sürer, bir sonraki sınır satırında kapanır; **bölmeyen** kolon aynı yükseklikte kesiksiz akar ve `Parça 73 sürüyor` der. Bu iki kolonun ayrıştığı satır **ayrışma**dır: solundaki numaralı düğme oraya götürür, üstteki tek satır olayı söyler ("Deep Analysis burada yeni parça açtı · Standard devam etti") ve varsa Deep'in o sınırdaki gerekçesi **Neden?** ile açılır. Ekran ilk ayrışmada açılır; ← → (ya da n / p) ayrışmadan ayrışmaya gezer ve ilgili parçaları görünür alana getirir. Üst şerit yalnız üç kontrol taşır: **yöntemler**, **← Ayrışma x / 43 →** ve **Sayfa**; metin yoğunluğu, "yalnız ayrışan bölümler", "Yalnız Standard ↔ Deep" ve sonuç bandına inen bağlantı tek bir **Görünüm** düğmesinin arkasındadır. Parça detayı ekranı sürekli kaplamaz: bir parçaya tıklandığında sağdan açılan bir çekmecedir. **Sonuç** bandı, "Ne düzeldi?" çubukları, yöntem kartları ve metodoloji tezgâhın altındadır |
