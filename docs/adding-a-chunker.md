...
1. **Yöntem modülünü yazın.** Girdi canonical birimler, çıktı yapısal
   ...
   ```python
   # src/amsc/fixed_window_chunker.py
   from .chunk_method import ChunkMethod, PartitionResult   # <- methods'tan DEĞİL
   ...
   **Tipler `amsc.chunk_method`'tan alınır, `amsc.methods`'tan değil.**
   `chunk_method` paketten hiçbir şey import etmeyen bir yaprak modüldür;
   `methods` sizin modülünüzü import edeceği için tipleri oradan almak
   döngü olur (`methods -> sizin modülünüz -> methods`) ve paketin import
   döngüsü muhafızını düşürür. `src/amsc/example_chunker.py` bu şekli
   birebir taşıyan, eksiksiz ve kopyalanabilir örnektir.

2. **Kaydedin.** `src/amsc/methods.py` içinde iki satır: import ve demete
   ekleme.

   ```python
   from .fixed_window_chunker import FIXED_WINDOW          # diğer importların yanında
   ...
...
`python -m amsc.viewer_v3` çalıştırmasını hatırlamasına bağlıydı — kimsenin
hata vermediği, yalnız yöntemin görünmediği sessiz bir adım.

Artık sayfa sunulduğunda açılışta `GET /api/methods` çağırır
(`amsc.viewer_server`), yanıtı gömülü kopyanın üzerine yazar ve değişmişse
yeniden çizer. Sunucu kaydı `amsc.methods`'ten okur, yani kütüphanede kayıtlı
...
Tutan test:
`tests/unit/test_methods_registry.py::test_a_page_built_before_the_method_existed_still_lists_it_when_served`.
...
değiştirin, `ChunkMethod`'u `amsc/methods.py` içine import edip `_BUILTIN`
