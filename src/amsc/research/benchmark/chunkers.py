...
from :func:`amsc.research.benchmark.retrieval._evaluate_candidate`, imported and called
...
* the index must return :class:`amsc.retrieval.pipeline.RetrievalHit`. The
...
frozen :class:`amsc.retrieval.pipeline.DeterministicBM25` over a Turkish
...
from ...quality import chunks as chunk_quality
...
from ...chunking.adaptive.config import V4Config
from ...embedding.boundary import CachedSemanticBoundaryEmbedder, SentenceTransformerBoundaryEmbedder
from ...quality.evaluation import load_jsonl_objects
from ...document.io import load_jsonl_units, sha256_file
...
from ...retrieval.pipeline import DeterministicBM25, RetrievalDocument, RetrievalHit
from ...document.tokenization import TiktokenTokenCounter, TokenCounter
...
class ArmConfig(_Strict):
    #: A registered partition method's kind (``amsc.chunking.registry``). Validated
    ...
    def validate_kind_fields(self) -> "ArmConfig":
        try:
            method = registry.by_kind(self.kind)
        except registry.UnknownMethod as unknown:
...
def run_arm(
    ...
    *,
    ...
    partition method and :func:`amsc.chunking.registry.partition` runs it. Nothing here
    knows which engine is which; a registered method is a benchmarkable one.
    """
    settings = config.arms[arm]
    method = registry.by_kind(settings.kind)
    ...
    *,
    ...
    for arm in ARMS:
        ...
        timing[arm] = {
            ...
            "uses_embeddings": registry.get(arm).needs_embedder,
            "cold_equals_warm": not registry.get(arm).needs_embedder,
        }
        if registry.get(arm).needs_embedder and measure_cold_embedding_time:
...
def _parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        prog="python -m amsc.research.benchmark.chunkers",
        description=(
