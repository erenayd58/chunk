...
* ``change_group`` -- a span between cuts common to both partitions where
  they differ (from :func:`amsc.quality.boundaries.compare`). The annotator
...
walk, pinned against :func:`amsc.chunking.structural.chunk_units` by test.
"""
...
from ...quality.boundaries import (
    QualityConfig,
    boundary_smells,
    compare,
    load_rows,
...
from ...chunking.mapping import base_unit_id
...
def enumerate_windows(
    ...
    *,
    ...
    A step-for-step mirror of ``chunking.structural.chunk_units`` up to the
