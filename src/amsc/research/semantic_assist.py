...
chunking, :mod:`amsc.research.agentic.judge`): boundary decisions on important
...
:func:`amsc.chunking.structural.chunk_units` unchanged, ``SEMANTIC_ASSIST``
delegates to :func:`amsc.chunking.hybrid.chunk_units`, whose H1 arbitration is
...
The provider contract is :class:`amsc.chunking.hybrid.BoundaryEmbedder` --
...
from ..chunking import hybrid
from ..chunking import structural
from ..chunking.hybrid import BoundaryEmbedder, _plan_cuts
from ..document.models import EmbeddingBatch, RawDocumentUnit
...
def chunk_with_mode(
    ...
    *,
    ...
    ``STANDARD`` is byte-identical to :func:`structural.chunk_units`
    ...
    if config.mode is ChunkingMode.STANDARD:
        chunks = structural.chunk_units(units, counter=counter, **budgets)
        return ModeResult(
            mode=config.mode,
            chunks=chunks,
            diagnostics={
