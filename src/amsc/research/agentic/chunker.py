...
3. **Parse + coherence guard** (deterministic): the strict window parser is
   ...
   (``coherence_violation``). Deep Analysis (:mod:`amsc.research.agentic.judge`)
...
4. **Vote-guided walk** (deterministic): the structural walk, byte for
   ...
   output is byte-identical to :func:`amsc.chunking.structural.chunk_units`.
...
from ...chunking.structural import (
    Piece,
    Section,
    _render,
    _sections,
)
from ...chunking.structural import chunk_units as structural_chunk_units
from ...document.tokenization import TokenCounter
...
# --------------------------------------------------------------------------
# phase 2: parallel vote collection -- :func:`amsc.providers.collect_votes`,
...
def _walk_section(
    ...
    *,
    ...
    otherwise a step-for-step mirror of ``chunking.structural.chunk_units``
...
def chunk_units_agentic(
    ...
    *,
    ...
    ``chunking.structural.chunk_units`` (chunk ids aside)."""
...
def build_artifact(
    *,
    ...
    from ..benchmark.chunkers import normalize_unit_ids_for_retrieval
    from ...chunking.mapping import map_chunks
