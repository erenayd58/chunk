...
writes the files the viewer and the benchmark panel read:
    ...
    <deep_tree>/arm/mapping.json             chunk <-> unit resolution (amsc.chunking.mapping)
...
:mod:`amsc.research.benchmark.agentic`'s ``_evaluate_gold`` -- the same BM25 settings,
...
the frozen evaluation stack (``agentic_benchmark`` -> ``research.benchmark.chunkers`` ->
``research.benchmark.retrieval`` -> ``research.legacy_chat_rag``). See :mod:`amsc.surface`.
"""
...
from ..quality import boundaries as bq
from ..quality import chunks as chunk_quality
from . import selector as da
...
from .pipeline import run_standard
from ..document.io import load_jsonl_units, sha256_file
from ..document.models import RawDocumentUnit
from ..chunking.structural import _sections
from ..document.tokenization import TiktokenTokenCounter, TokenCounter

ARM_KIND = registry.DEEP_KIND
GENERATOR = "amsc.deep.arm"
...
def _package_rows(
    ...
    *,
    ...
    if frozen_tree is not None:
        ...
        # set), and ``amsc.research.benchmark.agentic`` reaches the whole frozen
        ...
        from ..research.benchmark.agentic import _evaluate_gold
