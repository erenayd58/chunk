...
* **only boundaries the deterministic layer would accept are marked.** A cut
  ...
  :mod:`amsc.quality.boundaries`, so offering it invites an answer nobody can
  act on. This removes roughly a third of the markers.
* **the model never sees the partition.** It scores boundaries; the DP in
  :mod:`amsc.deep.selector` picks the partition, and strength enters that
...
from ..providers import BoundaryJudgeModel, CallOutcome, collect_votes
from ..chunking.structural import Piece, Section, _sections
