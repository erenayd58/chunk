...
in the browser and no second data path.
    ...
    GET  /                      the viewer page (built by amsc.viewer.build)
...
from ..chunking import registry as method_registry
from .chat.session import Catalog, ChatEngine, load_config
...
def method_registry_payload() -> dict[str, Any]:
    ...
    from the Viewer. Read from :mod:`amsc.chunking.registry`, never restated.
    """
    return {
        ...
        "generator": "amsc.chunking.registry",
    }
...
def main(argv: Sequence[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        prog="python -m amsc.viewer.server",
