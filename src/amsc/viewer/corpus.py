"""The Viewer's document reader -- one payload shape, read from artifact trees.

This is the data layer both Viewer pages are built on, and the contract the
RAG console packages live documents to. It owns exactly one thing: turning
completed artifact trees into the per-document payload a Viewer page renders
(:func:`load_corpus`) and the arm index the chat server serves
(:func:`catalog`). It renders no page and it knows about no page.

Who reads it:

* :mod:`amsc.viewer.build` -- the product page. Embeds one payload per document
...
* ``chat_rag`` (``components/viewer/analysis.py``) -- the console's packaging
  ...
  exactly the shape a frozen one does. That is the whole cross-repository
  contract: no second reader, no second shape.

Inputs, per document:

* a frozen ``amsc.research.benchmark.chunkers`` tree (three arms, gold queries, pinned
  canonical) -- optional;
* a packaged ``amsc.deep.run`` tree (``arm/``, ``standard/``,
  ``boundary-decisions.json``) -- optional, adds Deep Analysis;
* ``extra_arm_dirs`` -- any further method packaged by ``amsc.deep.arm``,
...
``amsc.deep.arm`` derived. **The differences filter** is defined over
...
Method identity is never decided here -- it is read from :mod:`amsc.chunking.registry`,
...
from ..chunking.relations import continuation_groups, derive_continuations
...
ARM_ORDER = registry.benchmark_arms()
PRODUCT_ARM_ORDER = registry.ORDER
ARM_LABELS = registry.LABELS
...
def _require(path: Path) -> Path:
    if not path.is_file():
        raise ValueError(
            ...
            "amsc.research.benchmark.chunkers output tree and cannot invent the data"
        )
    return path
...
def _attach_links(chunks: list[dict], chunks_raw: Sequence[dict], kind: str) -> None:
    ...
    # amsc.chunking.relations (single source of truth) over the same frozen
...
def _load_deep_arm(
    ...
    """The Deep Analysis arm from a packaged ``amsc.deep.run`` tree.
    ...
    if manifest.get("arm_kind") != "deep_analysis":
        raise ValueError(f"{arm_dir} is not a packaged Deep Analysis arm (run amsc.deep.arm)")
...
# --------------------------------------------------------------------------
# the catalog: which chunk files exist, for the chat server
# --------------------------------------------------------------------------
...
def catalog(
    ...
    root: Path,
    generator: str = "amsc.viewer.corpus",
) -> dict:
    """What the chat server serves: exactly the arms the HTML shows.

    ``generator`` names the page build that wrote it, so a catalog beside a
    page says which builder produced the pair.
    """
    ...
    return {"generator": generator, "documents": documents, "build": build}
