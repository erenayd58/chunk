...
:mod:`amsc.chunking.mapping` result -- never from a field only one chunker writes.
...
* **Most of what** :func:`amsc.quality.lint.lint` **finds is a property of the
...
from ..chunking.mapping import DocumentMapping, Segment
...
def qa_view(chunks: Sequence[Mapping[str, Any]]) -> list[dict[str, Any]]:
    ...
    ``quality.lint`` predates markdown rendering: ``strip_emphasis`` removes
